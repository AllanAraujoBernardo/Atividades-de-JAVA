...
public class Carrinho {
    ...
    public List<String> findNomesRepetidos() {
        List<String> produtosRepetidos = new ArrayList<>();

        for (int i = 0; i < produtos.size(); i++) {
            for (int j = 0; j < produtos.size(); j++) {
                if (produtos.get(i).getNome().equalsIgnoreCase(produtos.get(j).getNome()) && (i != j)) {
                    produtosRepetidos.add(produtos.get(i).getNome());
                }
            }
        }

        for (int k = 0; k < produtosRepetidos.size(); k++) {
            for(int l = 0; l < produtosRepetidos.size(); l++){
            if (produtosRepetidos.get(k).equalsIgnoreCase(produtosRepetidos.get(l)) && (k != l)) {
                produtosRepetidos.remove(k);
            }
            }
        }
